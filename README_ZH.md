...
**FLUID —— 流态 / 生成式 UI 研究** —— 一个面向「智能体驱动的 UI 操作系统」的知识库：由 AI 智能体在运行时组合界面，而不是交付固定的页面——可以完全本地运行（llama.cpp / vLLM / Ollama）、接云端模型（Claude、Gemini、GPT），也可以混合。

- **范围** —— 167 篇互相链接、标注来源的文档，涵盖协议（A2UI、MCP、AG-UI）、架构、可靠的 UI 生成、状态、安全、延迟与成本——另有一份浏览器端流态桌面的方案：一次输入就组合出你当下需要的应用。
- **进度** —— 这是写下第一行代码之前的研究与决策。每条论断都标注为已核实、部分核实或推测，其中多数为部分核实。目前是设计，不是已上线的系统。
