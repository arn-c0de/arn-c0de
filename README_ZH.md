...
个人重点：嵌入式系统、后端部署、服务器集成、Android 应用与 Web 应用。
...
**[Project Nova / Hashkrieg](https://github.com/VibecodingGermany/Project_Nova) 的 [Nova.AiLab](https://github.com/arn-c0de/Nova.AiLab)** —— 我正在开发的模拟环境，用于比较来自不同分支的 AI 版本。它评估自适应 RTS 行为，包括 NPC 目标、经济、军队构成、扩张、防守、进攻、攻城和移动。实验室结果只是诊断；真实对局中未观察到的效果会保持标记。

**FLUID** —— 正在积极实现和测试的智能体驱动流态/生成式 UI 操作系统，涵盖 A2UI、MCP、AG-UI、架构、状态、安全、延迟与成本。通过应用和 Harness 测试本地、云端及混合系统，并由有来源链接的知识库提供支持。

**ARIS** —— 自托管智能体，支持完整的流式合成语音、多模型接入，以及用于工具调用和文件编辑的可扩展 Harness。面向本地边缘硬件设计；在可用时使用更强的模型服务器，不可用时平稳降级运行。

**DeepVision-Web** —— 私有、闭源的独立移动端 PCB 视觉追溯试点项目，可直接在设备上运行。它结合 WebAssembly 跟踪、基于相机的 PnP 叠加层、参考校准和只读 Trace 适配器；相机帧始终留在设备上。
